...
### Droits des dossiers du banc

Le sous-dossier `Tracks/PiPedalAI` doit être préparé avant la première séance :
PiPedal doit pouvoir lire la copie DI. Les DI originales, préécoutes et journaux
restent privés.

Depuis la 0.8.5, le banc récupère le nom réellement produit par TooB Record Input
via `getPatchProperty`, puis vérifie le chemin, le format et la nouveauté du fichier.
Il attend jusqu'à 15 secondes la finalisation après Stop, avant restauration.
TooB génère un fichier `rec-*.wav` dans le dossier PiPedal `audiorecording` :
`record_directory` doit désigner ce dossier réel. La propriété `audioFile` sert à
sélectionner une prise existante et ne permet pas de choisir le chemin de sortie.
Le dossier `Audio Recordings/PiPedalAI` préparé en 0.8.4 n'est donc plus utilisé.
...
défaut conserve le partage des fichiers dans ce sous-dossier. La copie DI
...
PiPedal AI doit aussi pouvoir parcourir `record_directory` et lire le fichier
produit par PiPedal. Il n'a pas besoin d'y écrire. Si la prise n'est pas lisible,
l'erreur indique son chemin exact ; vérifie alors ses droits avec `stat` et
`getfacl`. Le banc ne remplace pas ce fichier par une autre prise trouvée sur disque.
...
le sous-dossier de lecture DI en écriture :
